...
test("a different listing pays the full bid, not another listing's difference", async (t) => {
  const polar = new FixturePolar();
  const app = await buildApp({ polar });
  t.after(() => app.close());
  insertIssue(app.db, OPEN_ISSUE, "open");
  ...
  const first = await payListing(app, polar, {
    sponsorUrl: "https://cover.example",
    blurb: "Cover at twenty",
    bidUsd: 20,
  });
  assert.equal(first.amountUsd, 20);

  const other = await startListingCheckout(app.db, polar, {
    sponsorUrl: "https://other.example",
    blurb: "Different URL pays the full eight",
    bidUsd: 8,
  });
  assert.notEqual(other.listingId, first.listingId);
  assert.equal(other.amountUsd, 8);
  assert.equal(other.targetBidUsd, 8);
  assert.equal(listingCount(app.db), 2);

  await completeCheckout(app.db, polar, other.polarCheckoutId);
  const stored = findListingById(app.db, other.listingId);
  assert.ok(stored);
  assert.equal(stored.bidUsd, 8);
  const board = await boardListings(app);
  assert.deepEqual(
    board.map((row) => ({ id: row.id, rank: row.rank, bidUsd: row.bidUsd })),
    [
      { id: first.listingId, rank: 1, bidUsd: 20 },
      { id: other.listingId, rank: 2, bidUsd: 8 },
    ],
  );
});
